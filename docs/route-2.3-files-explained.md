...
- **用途（工程角度）**：首頁入口；現在本身只是一個「組裝頁」，實際 UI 拆到 `components/home/` 底下的分段元件：
  - `HomeSplashGate`：首頁用的 Splash 掛載點（吃 `authStore.isLoading`）。
  - `HomeHeroSection`：Hero + CTA（登入 / 訪客進入）。
  - `HomeHighlightsSection`：熱門遊戲 + 精選商品版位。
  - `HomeBrainrotHintSection`：最下方的 Italian Brainrot 提示區。
- **白話文**：這就是「你進站第一眼看到的首頁」，現在把不同區塊拆成小元件，方便之後各自接上真實遊戲/商品資料與動畫。
